#!/bin/sh
# Tiny HyperbolaBSD stage-2 smoke: build libc.so and run a dynamic hello
# Usage:
#   sh tools/smoke-hyperbolabsd-stage2.sh
#   MUSL_USE_MUSL_LDSO=1 sh tools/smoke-hyperbolabsd-stage2.sh   # force musl ldso in PT_INTERP
set -eu

echo "[$0] building shared libc..."
${MAKE:-make} -j"$(sysctl -n hw.ncpu 2>/dev/null || echo 4)" lib/libc.so

echo "[$0] ensuring ldso alias..."
${MAKE:-make} ldso-symlink

# Ensure musl CRT objects exist (do not use system /usr/lib/crt*.o)
echo "[$0] ensuring musl CRT objects..."
${MAKE:-make} -j"$(sysctl -n hw.ncpu 2>/dev/null || echo 4)" \
...
case "$os" in
  HyperbolaBSD)
    if [ "$use_musl_ldso" = "1" ]; then
      ...
      echo "[$0] NOTE: musl ldso selected by default; HyperbolaBSD will not exec non-system PT_INTERP (ENOEXEC)."
...
if [ "$rc" -eq 0 ]; then
  ...
  if [ "$os" = "HyperbolaBSD" ] && [ "$use_musl_ldso" = "1" ]; then
    echo "Note: HyperbolaBSD does not exec non-system interpreters; failure is expected when forcing musl ldso."
  fi
fi

echo "----- try explicit musl loader -----"
# On HyperbolaBSD this usually fails with ENOEXEC; treat as best-effort.
