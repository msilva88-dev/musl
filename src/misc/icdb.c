...
struct icdb *__icdb_new(uint32_t version, uint32_t nentries, uint32_t entrysize,
...
weak_alias(__icdb_new, icdb_new);

struct icdb *__icdb_open(const char *name, int flags, uint32_t version)
{
	struct icdb *db = NULL;
	struct icdbinfo *info;
	struct stat sb;
...
weak_alias(__icdb_open, icdb_open);

int __icdb_get(struct icdb *db, void *entry, uint32_t idx)
{
...
weak_alias(__icdb_get, icdb_get);

int __icdb_lookup(struct icdb *db, int keynum, const void *key, void *entry, uint32_t *idxp)
{
...
weak_alias(__icdb_lookup, icdb_lookup);

int __icdb_nentries(struct icdb *db)
{
	return db->info->nentries;
}
weak_alias(__icdb_nentries, icdb_nentries);

const void *__icdb_entries(struct icdb *db)
{
	return db->entries;
}
weak_alias(__icdb_entries, icdb_entries);

int __icdb_update(struct icdb *db, const void *entry, int offset)
{
	struct icdbinfo *info = db->info;
...
weak_alias(__icdb_update, icdb_update);

int __icdb_add(struct icdb *db, const void *entry)
{
	struct icdbinfo *info = db->info;
...
weak_alias(__icdb_add, icdb_add);

int __icdb_rehash(struct icdb *db)
{
	struct icdbinfo *info = db->info;
	uint32_t entrysize = info->entrysize;
	uint32_t indexsize, idxmask, idxlen;
...
weak_alias(__icdb_rehash, icdb_rehash);

int __icdb_save(struct icdb *db, int fd)
{
	struct icdbinfo *info = db->info;
	uint32_t entrysize = info->entrysize;
...
weak_alias(__icdb_save, icdb_save);

int __icdb_close(struct icdb *db)
{
	int i;
...
weak_alias(__icdb_close, icdb_close);
