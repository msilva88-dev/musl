...
struct ber_element *ober_get_element(unsigned int encoding)
{
...
void ober_set_header(struct ber_element *elm, int class, unsigned int type)
{
...
void ober_link_elements(struct ber_element *prev, struct ber_element *elm)
{
	if (prev != NULL) {
...
struct ber_element *ober_unlink_elements(struct ber_element *prev)
{
	struct ber_element *elm;
...
void ober_replace_elements(struct ber_element *prev, struct ber_element *new)
{
...
struct ber_element *ober_add_sequence(struct ber_element *prev)
{
	struct ber_element *elm;
...
struct ber_element *ober_add_set(struct ber_element *prev)
{
	struct ber_element *elm;
...
struct ber_element *ober_add_enumerated(struct ber_element *prev, long long val)
{
	struct ber_element *elm;
...
struct ber_element *ober_add_integer(struct ber_element *prev, long long val)
{
	struct ber_element *elm;
	unsigned int i, len = 0;
	unsigned char cur, last = 0;
...
int ober_get_integer(struct ber_element *elm, long long *n)
{
...
int ober_get_enumerated(struct ber_element *elm, long long *n)
{
...
struct ber_element *ober_add_boolean(struct ber_element *prev, int bool)
{
	struct ber_element *elm;
...
int ober_get_boolean(struct ber_element *elm, int *b)
{
...
struct ber_element *ober_add_string(struct ber_element *prev, const char *string)
{
	return ober_add_nstring(prev, string, strlen(string));
}

struct ber_element *ober_add_nstring(struct ber_element *prev, const char *string0, size_t len)
{
	struct ber_element *elm;
	char *string;
...
struct ber_element *ober_add_ostring(struct ber_element *prev, struct ber_octetstring *s)
{
	return ober_add_nstring(prev, s->ostr_val, s->ostr_len);
}

int ober_get_string(struct ber_element *elm, char **s)
{
...
int ober_get_nstring(struct ber_element *elm, void **p, size_t *len)
{
	if (elm->be_encoding != BER_TYPE_OCTETSTRING)
		return -1;
...
int ober_get_ostring(struct ber_element *elm, struct ber_octetstring *s)
{
	if (elm->be_encoding != BER_TYPE_OCTETSTRING)
		return -1;
...
struct ber_element *ober_add_bitstring(struct ber_element *prev, const void *v0, size_t len)
{
	struct ber_element *elm;
	void *v;
...
int ober_get_bitstring(struct ber_element *elm, void **v, size_t *len)
{
...
struct ber_element *ober_add_null(struct ber_element *prev)
{
	struct ber_element *elm;
...
int ober_get_null(struct ber_element *elm)
{
...
struct ber_element *ober_add_eoc(struct ber_element *prev)
{
	struct ber_element *elm;
...
int ober_get_eoc(struct ber_element *elm)
{
...
size_t ober_oid2ber(struct ber_oid *o, uint8_t *buf, size_t len)
{
	uint32_t v;
...
int ober_string2oid(const char *oidstr, struct ber_oid *o)
{
...
int ober_oid_cmp(struct ber_oid *a, struct ber_oid *b)
{
	size_t i;
...
struct ber_element *ober_add_oid(struct ber_element *prev, struct ber_oid *o)
{
	struct ber_element *elm;
	uint8_t *buf;
	size_t len;
...
struct ber_element *ober_add_noid(struct ber_element *prev, struct ber_oid *o, int n)
{
	struct ber_oid no;
...
int ober_get_oid(struct ber_element *elm, struct ber_oid *o)
{
	uint8_t *buf;
	size_t len, i = 0, j = 0;
...
struct ber_element *ober_printf_elements(struct ber_element *ber, char *fmt, ...)
{
	va_list ap;
	int d, class;
	size_t len;
...
int ober_scanf_elements(struct ber_element *ber, char *fmt, ...)
{
#define _MAX_SEQ 128
	va_list ap;
	int *d, level = -1;
...
ssize_t ober_get_writebuf(struct ber *b, void **buf)
{
	if (b->br_wbuf == NULL)
		return -1;
	*buf = b->br_wbuf;
 ...
 */
ssize_t ober_write_elements(struct ber *ber, struct ber_element *root)
{
	size_t len;
...
void ober_set_readbuf(struct ber *b, void *buf, size_t len)
{
 ...
 */
struct ber_element *ober_read_elements(struct ber *ber, struct ber_element *elm)
{
...
off_t ober_getpos(struct ber_element *elm)
{
	return elm->be_offs;
}

struct ber_element *ober_dup(struct ber_element *orig)
{
...
void ober_free_element(struct ber_element *root)
{
...
void ober_free_elements(struct ber_element *root)
{
	if (root == NULL)
		return;
	if (root->be_sub && (root->be_encoding == BER_TYPE_SEQUENCE ||
...
size_t ober_calc_len(struct ber_element *root)
{
	unsigned int t;
	size_t s;
...
void ober_set_application(struct ber *b, unsigned int (*cb)(struct ber_element *))
{
	b->br_application = cb;
}

void ober_set_writecallback(struct ber_element *elm, void (*cb)(void *, size_t),
	void *arg)
{
	elm->be_cb = cb;
	elm->be_cbarg = arg;
 ...
 */
static ssize_t get_id(struct ber *b, unsigned int *tag, int *class, int *cstruct)
{
	unsigned char u;
	size_t i = 0;
	unsigned int t = 0;
