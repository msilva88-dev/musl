...
/* pkcs5 pbkdf2 from OpenBSD 7.0 source code: lib/libutil/pkcs5_pbkdf2.c */
 ...
 */
static void hmac_sha1(const uint8_t *text, size_t text_len, const uint8_t *key,
	...
	size_t salt_len, uint8_t *key, size_t key_len, unsigned int rounds)
{
	...
	for (count = 1; key_len > 0; count++) {
		...
		hmac_sha1(asalt, salt_len + 4, (const uint8_t *)pass,
		    pass_len, d1);
		memcpy(obuf, d1, sizeof(obuf));

		for (i = 1; i < rounds; i++) {
			hmac_sha1(d1, sizeof(d1), (const uint8_t *)pass,
			    pass_len, d2);
