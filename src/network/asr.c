...
void *__asr_resolver_from_string(const char *str)
{
	struct asr_ctx *ac;
...
weak_alias(__asr_resolver_from_string, asr_resolver_from_string);

void __asr_resolver_free(void *arg)
{
	_asr_ctx_unref(arg);
}
weak_alias(__asr_resolver_free, asr_resolver_free);

/*
 * Cancel an async query.
 */
void asr_abort(struct asr_query *as)
{
	_asr_async_free(as);
}
 ...
 * memory at "ar".
 */
int __asr_run(struct asr_query *as, struct asr_result *ar)
{
...
weak_alias(__asr_run, asr_run);

static int poll_intrsafe(struct pollfd *fds, nfds_t nfds, int timeout)
{
 ...
 */
int __asr_run_sync(struct asr_query *as, struct asr_result *ar)
{
	struct pollfd fds[1];
	int r, saved_errno = errno;
...
weak_alias(__asr_run_sync, asr_run_sync);
 ...
 * is running.
 */
struct asr_query *_asr_async_new(struct asr_ctx *ac, int type)
{
	struct asr_query *as;
 ...
 */
void _asr_async_free(struct asr_query *as)
{
 ...
 * using this context.
 */
struct asr_ctx *_asr_use_resolver(void *arg)
{
	struct asr_ctx *ac = arg;
	struct asr *asr;
	struct asr *priv;
...
static void asr_ctx_ref(struct asr_ctx *ac)
{
 ...
 * count drops to 0.
 */
void _asr_ctx_unref(struct asr_ctx *ac)
{
...
static void asr_ctx_free(struct asr_ctx *ac)
{
	int i;
 ...
 */
static void asr_check_reload(struct asr *asr)
{
	struct asr_ctx	*ac;
	struct stat	 st;
	struct timespec	 ts;
 ...
 * error.
 */
size_t _asr_make_fqdn(const char *name, const char *domain, char *buf, size_t buflen)
{
	size_t len;
 ...
 */
static int asr_ndots(const char *s)
{
	int n;
 ...
 */
static int asr_ctx_add_searchdomain(struct asr_ctx *ac, const char *domain)
{
	char buf[MAXDNAME];
...
static int strsplit(char *line, char **tokens, int ntokens)
{
	int ntok;
	char *cp, **tp;
 ...
 */
static void pass0(char **tok, int n, struct asr_ctx *ac)
{
	int i, j, d;
	const char *e;
 ...
 */
static int asr_ctx_from_string(struct asr_ctx *ac, const char *str)
{
	char buf[512], *ch;
 ...
 */
static int asr_ctx_from_file(struct asr_ctx *ac, const char *path)
{
	FILE *cf;
	char buf[4096];
	ssize_t r;
 ...
 */
static int asr_ctx_parse(struct asr_ctx *ac, const char *str)
{
	size_t len;
	const char *line;
	char buf[1024];
 ...
 */
static void asr_ctx_envopts(struct asr_ctx *ac)
{
	char buf[4096], *e;
	size_t s;
 ...
 */
static int asr_parse_nameserver(struct sockaddr *sa, const char *s)
{
	in_port_t portno = 53;
 ...
 */
int _asr_parse_namedb_line(FILE *file, char **tokens, int ntoken, char *lbuf, size_t sz)
{
	size_t len;
	char *buf;
	int ntok;
 ...
 */
int _asr_iter_db(struct asr_query *as)
{
