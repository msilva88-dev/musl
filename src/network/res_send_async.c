...
/* res_send_async from OpenBSD 7.0 source code: lib/libc/asr/res_send_async.c */
...
struct asr_query *__res_send_async(const unsigned char *buf, int buflen, void *asr)
{
	struct asr_ctx *ac;
	struct asr_query *as;
	struct asr_unpack p;
...
weak_alias(__res_send_async, res_send_async);
 ...
 */
struct asr_query *__res_query_async(const char *name, int class, int type, void *asr)
{
	struct asr_ctx *ac;
	struct asr_query *as;
...
weak_alias(__res_query_async, res_query_async);

struct asr_query *_res_query_async_ctx(const char *name, int class, int type, struct asr_ctx *a_ctx)
{
	struct asr_query *as;
...
static int res_send_async_run(struct asr_query *as, struct asr_result *ar)
{
    next:
	switch (as->as_state) {
...
static int sockaddr_connect(const struct sockaddr *sa, int socktype)
{
	int errno_save, sock;
 ...
 */
static int setup_query(struct asr_query *as, const char *name, const char *dom,
	int class, int type)
{
	struct asr_pack p;
	struct asr_dns_header h;
 ...
 */
static int udp_send(struct asr_query *as)
{
	ssize_t	n;
	int save_errno;
#ifdef DEBUG
 ...
 */
static int udp_recv(struct asr_query *as)
{
	ssize_t n;
	int save_errno;
 ...
 */
static int tcp_write(struct asr_query *as)
{
	struct msghdr msg;
	struct iovec iov[2];
	uint16_t len;
 ...
 */
static int tcp_read(struct asr_query *as)
{
	ssize_t n;
	size_t offset, len;
	char *pos;
 ...
 */
static int ensure_ibuf(struct asr_query *as, size_t n)
{
	char *t;
 ...
 */
static int validate_packet(struct asr_query *as)
{
	struct asr_unpack p;
	struct asr_dns_header h;
	struct asr_dns_query q;
 ...
 */
static int iter_ns(struct asr_query *as)
{
	for (;;) {
