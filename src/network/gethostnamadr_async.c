...
/* gethostnamadr_async from OpenBSD 7.0 source code: lib/libc/asr/gethostnamadr_async.c */
...
static struct hostent_ext *hostent_file_match(FILE *, int, int, const char *, int);
static struct hostent_ext *hostent_from_packet(int, int, char *, size_t);
static void netent_from_hostent(struct asr_result *ar);

struct asr_query *__gethostbyname_async(const char *name, void *asr)
{
	return gethostbyname2_async(name, AF_INET, asr);
}
weak_alias(__gethostbyname_async, gethostbyname_async);

struct asr_query *__gethostbyname2_async(const char *name, int af, void *asr)
{
	struct asr_ctx *ac;
	struct asr_query *as;
...
weak_alias(__gethostbyname2_async, gethostbyname2_async);

struct asr_query *__gethostbyaddr_async(const void *addr, socklen_t len, int af, void *asr)
{
	struct asr_ctx *ac;
	struct asr_query *as;
...
weak_alias(__gethostbyaddr_async, gethostbyaddr_async);

struct asr_query *_gethostbyaddr_async_ctx(const void *addr, socklen_t len, int af,
	struct asr_ctx *ac)
{
	struct asr_query *as;
...
static int gethostnamadr_async_run(struct asr_query *as, struct asr_result *ar)
{
 ...
 */
static struct hostent_ext *hostent_from_addr(int family, const char *name, const char *addr)
{
	struct hostent_ext *h;
 ...
 */
static struct hostent_ext *hostent_file_match(FILE *f, int reqtype, int family, const char *data,
	int datalen)
{
 ...
 */
static struct hostent_ext *hostent_from_packet(int reqtype, int family, char *pkt, size_t pktlen)
{
	struct hostent_ext *h;
	struct asr_unpack p;
...
static struct hostent_ext *hostent_alloc(int family)
{
	struct hostent_ext *h;
	size_t alloc;
...
static int hostent_set_cname(struct hostent_ext *h, const char *name, int isdname)
{
	char buf[MAXDNAME];
	size_t n;
...
static int hostent_add_alias(struct hostent_ext *h, const char *name, int isdname)
{
	char buf[MAXDNAME];
	size_t i, n;
...
static int hostent_add_addr(struct hostent_ext *h, const void *addr, size_t size)
{
	int i;
