...
static const char *rcodetostr(uint16_t v)
{
	switch (v) {
...
static const char *print_dname(const char *_dname, char *buf, size_t max)
{
	return _asr_strdname(_dname, buf, max);
}

static const char *print_rr(const struct asr_dns_rr *rr, char *buf, size_t max)
{
	char *res;
	char tmp[256];
	char tmp2[256];
...
static const char *print_query(const struct asr_dns_query *q, char *buf, size_t max)
{
	char b[256];
...
static const char *print_header(const struct asr_dns_header *h, char *buf, size_t max)
{
	snprintf(buf, max,
...
void _asr_dump_packet(FILE *f, const void *data, size_t len)
{
	char buf[1024];
	struct asr_unpack p;
...
const char *_asr_print_sockaddr(const struct sockaddr *sa, char *buf, size_t len)
{
	char h[256];
	int portno;
	union {
...
void _asr_dump_config(FILE *f, struct asr *a)
{
	char buf[256];
	int i;
	struct asr_ctx *ac;
...
const char *_asr_statestr(int state)
{
	switch (state) {
	CASE(ASR_STATE_INIT);
...
const char *_asr_querystr(int type)
{
	switch (type) {
	CASE(ASR_SEND);
	CASE(ASR_SEARCH);
...
const char *_asr_transitionstr(int type)
{
	switch (type) {
	CASE(ASYNC_COND);
	CASE(ASYNC_DONE);
