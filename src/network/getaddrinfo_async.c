...
/* getaddrinfo_async from OpenBSD 7.0 source code: lib/libc/asr/getaddrinfo_async.c */
...
struct asr_query *__getaddrinfo_async(const char *hostname, const char *servname,
...
weak_alias(__getaddrinfo_async, getaddrinfo_async);

static int getaddrinfo_async_run(struct asr_query *as, struct asr_result *ar)
{
	char fqdn[MAXDNAME];
	const char *str;
	struct addrinfo *ai;
 ...
 */
static int iter_family(struct asr_query *as, int first)
{
	if (first) {
		as->as_family_idx = 0;
 ...
 */
static int addrinfo_add(struct asr_query *as, const struct sockaddr *sa, const char *cname)
{
	struct addrinfo *ai;
	int i, port, proto;
...
static int addrinfo_from_file(struct asr_query *as, int family, FILE *f)
{
...
static int addrinfo_from_pkt(struct asr_query *as, char *pkt, size_t pktlen)
{
	struct asr_unpack p;
