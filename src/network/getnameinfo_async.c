...
/* getnameinfo_async from OpenBSD 7.0 source code: lib/libc/asr/getnameinfo_async.c */
...
struct asr_query *__getnameinfo_async(const struct sockaddr *sa, socklen_t slen, char *host,
...
weak_alias(__getnameinfo_async, getnameinfo_async);

static int getnameinfo_async_run(struct asr_query *as, struct asr_result *ar)
{
	void *addr;
	socklen_t addrlen;
	int r;
 ...
 */
static int _numerichost(struct asr_query *as)
{
	unsigned int ifidx;
