...
static struct _rsx {
	struct __chacha_ctx rs_chacha;	/* chacha context for random keystream */
...
static inline void _rs_init(unsigned char *buf, size_t n)
{
	if (n < KEYSZ + IVSZ)
		return;
...
hidden uint32_t __arc4random(void)
{
	uint32_t val;
...
hidden void __arc4random_buf(void *buf, size_t n)
{
	pthread_mutex_lock(&arc4random_mtx);
